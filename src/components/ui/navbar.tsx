  ...
  return (
    <header
      className={cn(
        ...
        isScrolled
          ? 'bg-blue-500/30 dark:bg-gray-900/80 backdrop-blur-md shadow-sm'
          : 'bg-transparent'
      )}
    >
      <nav className="container mx-auto flex items-center justify-between">
        ...
        >
          <span className="relative z-10 font-logo font-bold text-3xl text-gray-900">
            bink.dev
          </span>
        ...
        <ul className="hidden md:flex items-center space-x-8">
          {navItems.map((item) => (
            <li key={item.label}>
              <a
                href={item.href}
                className="relative font-semibold hover:text-foreground transition-colors duration-300 group"
              >
                {item.label}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-primary transition-all duration-300 group-hover:w-full"></span>
              </a>
